...
owner: "VII-Cae"
name: "hyalite--liquid-glass"
fullName: "VII-Cae/hyalite--liquid-glass"
description: "Real refraction liquid glass for the web: SDF lens maps + SVG displacement + backdrop-filter. One file, no WebGL."
sourceUrl: "https://github.com/VII-Cae/hyalite--liquid-glass"
stars: 117
forks: 7
language: "JavaScript"
topics: ["backdrop-filter", "css", "glassmorphism", "liquid-glass", "refraction", "svg-filters"]
license: "MIT"
homepage: "https://vii-cae.github.io/hyalite--liquid-glass/demo/index.html"
defaultBranch: "main"
snapshotDate: "2026-09-08"
pushedAt: "2026-09-07T08:55:03Z"
---
...
*图片：The lens on a grid: the field fans into the corners, nothing folds, nothing creases.*
...
|---|---|---|
| `bevel` | 16 | width of the bent zone along the edge, px. Clamped to the largest corner radius (see Corners) |
...
| `smooth` | 1 | px. Blur that hides Chromium’s nearest-neighbour staircase along the rim (see No stairs). It is applied *between* the two displacement passes and only inside the bevel ring; the centre never sees it. `0` goes back to a single pass — cheaper, and the stairs come back |
...
| `self` | false | the element filters *itself* (`filter: var(--hyalite)`) instead of its backdrop. Displacement only — see Gotchas |
...
- For a shape morph (a pill growing into a card): set `--hyalite: blur(px)` yourself while the shape moves, then `attach` with `materialize` once it lands. The map is built for a size; do not attach to an element that is still changing shape.

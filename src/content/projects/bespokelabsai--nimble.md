---
title: "bespokelabsai/nimble"
owner: "bespokelabsai"
name: "nimble"
fullName: "bespokelabsai/nimble"
description: "Local typed decisions, contrastive data curation, and model evaluation."
sourceUrl: "https://github.com/bespokelabsai/nimble"
stars: 555
forks: 41
language: "Python"
topics: []
...
[Model](https://huggingface.co/bespokelabs/Bespoke-Nimble-9B) · Capabilities · Quickstart · Methodology · Documentation and development · Citation

*图片：Introducing Bespoke Nimble. Serving reads the prompt once and then scores one answer token per question. Data curation changes one fact so that the correct answer flips. Training fine-tunes Qwen3.5-9B with LoRA on the answer tokens only. On 324 held-out examples, Bespoke-Nimble-9B matches 90.1% of the reference labels, compared with 66.4% for its base model and 93.2% for Jev 1.13.0.*
...
training evaluation. See the training guide for how
...
scoring guide for the schema rules and the ways you
can run the scorer.
...
*图片：Watch the contrastive data curation video: changing the signer from Mira to Noah flips the answer from true to false.*

Watch the 34-second explanation (silent).
...
generation and replay guide for the full
method.
...
See the dataset details and the
training guide. The curation guides also describe
...
*图片：Reference-label agreement on the same 324 examples: Jev 93.21%, Bespoke-Nimble-9B 90.12%, Qwen3.8-27B 84.88%, Qwen3.5-9B 66.36%, Qwen3.5-4B 61.42%, Qwen3.5-0.8B 45.37%, and Gemma 3 270M IT 28.70%.*
...
machine-readable comparison contains
...
training guide for the commands and for the contract
...
the public benchmarks guide, which runs Bespoke-Nimble-9B
...
| --- | --- |
| Define fields or understand parallel scoring | Scoring guide |
| Compare Nimble and Jev interactively | Comparison app |
| Host the published checkpoint on Modal | SGLang deployment · Try the public API |
| Understand the retained training and evaluation data | Dataset guide |
| Evaluate on public, human-labeled benchmarks | Public benchmarks |
| Create or replay contrastive training data | Curation guide |
| Train or use a schema adapter | Training guide |
...
The dependency lists are in requirements/. From the project

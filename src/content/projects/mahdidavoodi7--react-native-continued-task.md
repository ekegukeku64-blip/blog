---
title: "mahdidavoodi7/react-native-continued-task"
owner: "mahdidavoodi7"
name: "react-native-continued-task"
fullName: "mahdidavoodi7/react-native-continued-task"
description: "Background tasks that keep running after your app is backgrounded, for React Native and Expo. Wraps iOS 26 BGContinuedProcessingTask (Live Activity) and Android WorkManager foreground services behind one typed API. Swift + Kotlin via Nitro Modules."
sourceUrl: "https://github.com/mahdidavoodi7/react-native-continued-task"
stars: 128
forks: 2
language: "TypeScript"
topics: ["android", "background-processing", "background-task", "background-tasks", "background-upload", "backgroundtasks", "bgcontinuedprocessingtask", "bgtaskscheduler"]
license: "MIT"
homepage: "https://motionary.dev"
defaultBranch: "main"
snapshotDate: "2026-09-02"
pushedAt: "2026-09-01T12:07:26Z"
---
...
[*图片：npm*](https://www.npmjs.com/package/react-native-continued-task)
[*图片：npm downloads*](https://www.npmjs.com/package/react-native-continued-task)
*图片：license*
*图片：platforms*
[*图片：guide*](https://motionary.dev/blog/react-native-background-task)
...
It is built on [Nitro Modules](https://nitro.margelo.com) with Swift on iOS and Kotlin on Android, ships an Expo config plugin, and is written for the constraints these APIs actually impose rather than hiding them.
    ...
    iOS 26 — Live Activity on the Lock Screen
...
- ✅ Verified on a real device — 13/13 on iOS 26.6.1
...
Then `npx expo prebuild`. The plugin writes `BGTaskSchedulerPermittedIdentifiers` on iOS (expanding each prefix to `.*`), the GPU entitlement when `enableGPU` is set, and on Android the foreground-service permissions plus the merged `SystemForegroundService` block. See Config plugin for the full option list and the bare-workflow equivalents.
...
#### `submit(options: ContinuedTaskOptions): Promise`
...
#### `getKnownTasks(): Promise`

Every task this app has submitted that the library still holds a record for, newest first. Call it on launch — see Reconciling after a silent kill.
...
#### `attachToTask(id: string): Promise`
...
#### `forgetTasks(ids: string[]): Promise`
...
| `addOnStopListener(cb): ListenerSubscription`  | See Stop reasons                                   |
...
This is confirmed on hardware, not just read out of the header: cancelling from the Live Activity on iOS 26.6.1 reported `expired`. See the run log.
...
If the user declines, submitting still works and the task still runs — they simply cannot see or cancel it. The example app does this in `ensureNotificationPermission.ts`.
...
`BGTaskScheduler.submit(_:)` is reported as deprecated in iOS 27 in favor of `submitTaskRequest(_:completionHandler:)`. See SDK verification for what the shipping SDK actually exposes today and how this library is structured for it.
...
| ------------------------------- | -------------- | -------------------------------------------------------------------------------- |
| `identifierPrefixes`            | `[]`           | iOS `BGTaskSchedulerPermittedIdentifiers`, each expanded to `.*`         |
...
No `UIBackgroundModes` value is needed — see SDK verification.
...
| Manual device QA     | iPhone, iOS 26 | Everything about `BGContinuedProcessingTask` — **13/13 passing on iOS 26.6.1, 2026-09-01** | the example app                                                    |
...
The Harness suite in `harness/` is written and its runner starts, installs the app, boots it and bundles each test file on the emulator — but the tests do not execute yet. They fail with `ReferenceError: Property 'describe' doesn't exist`.
...
**See docs/DEVICE-QA.md for the full walkthrough** — signing, the bundle-identifier trap, and what each result means.
...
`STOP_REASON_QUOTA` seen in the wild. See CONTRIBUTING.md for the dev loop, and
docs/DEVICE-QA.md if you want to re-run the iOS checklist and add a run log.
...
No. `BGTaskScheduler` returns `.unavailable` on the Simulator — Apple documents this in the SDK header. `isSupported` reports `false` there and every submission rejects. iOS behaviour has to be verified on a physical device running iOS 26+; see docs/DEVICE-QA.md.
...
iOS cancels the task and — verbatim from Apple — "the app doesn't receive an indication of cancellation in that case." No stop listener, no expiration handler. Call `getKnownTasks()` on your next launch; work interrupted that way comes back with the `app-terminated` stop reason.
...
Almost certainly yes. On Android 13+ you must request `POST_NOTIFICATIONS` at runtime — without the grant the foreground service still starts and the work still runs, but the notification is suppressed. See Android 13+ needs the notification permission at runtime.
...
Yes. The Expo config plugin is a convenience; the Bare workflow section lists the `Info.plist` and `AndroidManifest.xml` entries to add by hand.

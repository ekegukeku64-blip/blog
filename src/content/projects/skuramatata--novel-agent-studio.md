---
title: "skuramatata/novel-agent-studio"
owner: "skuramatata"
name: "novel-agent-studio"
fullName: "skuramatata/novel-agent-studio"
description: "面向中文小说创作的本地桌面 AI 工作台：故事规划、分章写作、证据审稿、局部修订与故事记忆。"
sourceUrl: "https://github.com/skuramatata/novel-agent-studio"
stars: 35
forks: 6
language: "JavaScript"
topics: []
...
功能概览 · 界面与能力 · 快速开始 · 模型与数据 · 开发与架构 · 当前边界
...
*图片：创作对话：左侧作品导航，中间创作请求、章节候选与进度，右侧作者声音和作品上下文*
...
*图片：人物关系：可拖动的人物节点、有方向的关系连线，以及右侧关系详情*
...
*图片：故事记忆：章节索引统计、关键词和类型筛选，以及带原文引用的事件与知情记录*
...
*图片：创作日志：任务选择、执行状态、模型调用计数，以及可搜索和筛选的过程时间线*
...
`.env` 是本机配置，已被 Git 忽略；可以提交的是不含密钥的 `.env.example`。桌面开发版和命令行评测脚本默认读取项目根目录 `.env`。
...
`prepare:embedding` 按固定模型版本下载文件并校验 SHA-256；已有文件校验通过时直接复用。模型权重不提交到 Git，下载清单与上游说明保存在 `desktop-app/models/`。
...
浏览器访问 本地演示页面。该模式使用固定演示数据，不读取密钥、不调用模型，也不读写桌面应用中的真实作品。
...
当前适配 GLM 和 MiniMax，由 Electron 主进程直接请求所选供应商的官方接口。默认值来自 `runtime/catalog.mjs`：
...
“下载作品正文 (.md)”只导出已采纳章节的书名、标题和正文。“下载作品备份”导出作品 JSON，不包含全部运行日志、检查点和本地索引；完整迁移应在退出应用后备份整个应用数据目录。备份 JSON 可能包含作者真相、设定、对话和候选，分享前请核对内容。当前尚无备份导入界面，文件恢复步骤见桌面端存储与恢复说明。
...
构建和开发启动会自动递增应用版本，并生成包含北京时间与源码指纹的 `build-info.json`。界面左下角、桌面“关于”窗口与打包元信息使用同一份构建信息。打包后需要重新打开新版应用，已运行的旧进程不会自动更新。
...
- 桌面端使用说明：操作路径、配置优先级、存储恢复和评测命令。
- 模型预算与本地混合检索：分批审稿、段落修订和本地索引。
- 分层记忆与增量更新：记忆如何建立、回查和更新。
- 时间与事实连续性：时间、知情与证据的处理方式。
- 审稿恢复与作者确认：审稿、问答、复核与任务恢复。
- 创作日志：可观察的过程记录与历史任务查询。
- 作者档案与开写规划：作者声音、故事规划和人物设计。
- 评测协议：如何区分程序验证、模型审稿和文学质量评价。

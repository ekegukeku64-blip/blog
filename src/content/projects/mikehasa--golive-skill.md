...
owner: "mikehasa"
name: "golive-skill"
fullName: "mikehasa/golive-skill"
...
sourceUrl: "https://github.com/mikehasa/golive-skill"
stars: 951
forks: 65
language: "TypeScript"
...
- **You still approve every write.** Nothing reaches a real account without a plan you have seen and
  ...
  Trust, access and control separates what the code enforces from what is only an
...
- **A run stops rather than pushing on.** `apply` stops at the first failed check, missing
  ...
  and the next `apply` resumes at that step. Recovery covers
...
Those answers in full: trust, access and control and
recovery. The architecture is the product contract,
provider scope says what each provider can do today, the
validation record separates what has been exercised live from what is only
mock-covered, and distribution covers installation and updates.
...
connect accounts or deploy anything. See installation and updates for
...
that CLI as `npx golive@alpha `; its terminal commands run individual operations rather
...
you get the specific blocker and next action. See guided provider scope.
...
observed validation for the evidence.
...
#52: `email-verified` now resolves the records
...
path yet**, and other auth providers stay guided. See provider scope and
observed validation.
...
- [x] ✅ **Transactional email:** ~~Prove sending-domain setup, verification and real delivery with Resend.~~
  ...
  issue #52) still need validation.
...
- [ ] 🚧 **CI/CD and safe releases:** previews, release checks, promotion, rollback and drift
  ...
  made (`deployed::id` = `|||` in `.golive/state.json`,
...
- [x] ✅ **Uninstall / teardown:** ~~an approved inventory of golive-created resources and their removal.~~
  ...
  (#9). A removal forgets the baseline golive
...
Production checklists informing this roadmap
...
the host project — with reads taken now, and labels both sides: `expected (recorded by golive )`
...
- **Approve before account changes.** Plans name the destinations and intended writes, and `apply`
  ...
  the first time. Trust, access and control
...
See **CONTRIBUTING.md** for local setup, tests and your first contribution.
The architecture, trust, access and control,
recovery, provider scope and
validation record explain what exists and where help is needed.

MIT licensed. Bundled third-party notices are included in

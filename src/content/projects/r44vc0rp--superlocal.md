...
*图片：Unified inbox showing the included fictional mailboxes*
...
`sha-` tag, and the workflow records the digest for pinned deploys.
...
See the reader and mailbox settings

### Reading a conversation

*图片：A conversation from the fictional mock inbox*

### Choosing mailboxes and shortcuts

*图片：Per-user unified inbox selection and pinned mailbox shortcuts*
...
Runtime storage and advanced local configuration
...
| --- | --- |
| macOS | `~/Library/Application Support/superlocal//` |
| Linux | `$XDG_DATA_HOME/superlocal//`, defaulting to `~/.local/share` |
| Windows | `%LOCALAPPDATA%/superlocal//` |
...
The launcher serves the client on port **5178** and the SDK host on **8790**. `web.port`, `backend.port`, `web.origin`, and `web.allowedOrigins` control the local addresses. A null `web.origin` uses `http://localhost:`. An existing OpenCan setup can expose `https://super.local`; Superlocal does not install or manage OpenCan.

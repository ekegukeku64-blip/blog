...
fullName: "mizzlelover/gongwen-gbt9704-skill"
description: "跨平台中文公文排版 Skill，生成参照 GB/T 9704-2012 的可编辑 DOCX"
sourceUrl: "https://github.com/mizzlelover/gongwen-gbt9704-skill"
stars: 498
forks: 102
language: "JavaScript"
topics: ["ai-skill", "chinese-official-document", "claude-code", "cross-platform", "document-formatting", "docx", "gbt-9704-2012", "kimi"]
license: "MIT"
homepage: "https://x.com/dboy_yi2025"
defaultBranch: "main"
snapshotDate: "2026-09-12"
pushedAt: "2026-09-11T09:50:34Z"
---
...
2.0 重新核验了红头场景、首页预留、红线下标题间距、Word/WPS 标题样式、目录引用、特殊格式和跨平台安装，并完成 18 份 DOCX、37 个 PNG 页面和版头坐标量测。完整的更新说明见2.0 重大更新文章；文章、4 张新版插图和实际验证截图的合稿预览见2.0 图文排版预览。上一版记录仍保留在红头规则更新说明。

公众号发布可直接使用2.0 公众号 HTML：在浏览器打开后全选复制，再粘贴到公众号编辑器。
...
安装器将同一份源目录链接到各平台的用户级技能目录；Windows 默认复制以避免符号链接权限问题。TraeWork 使用可导入包 `dist/traework-gongwen-skill.zip`。完整目录、验证方法和官方依据见跨平台安装说明。

## 项目内容

- SKILL.md：使用指引与版式参数。
- 标准执行摘要：国家标准的可执行要点。
- 逐条执行矩阵：按条款记录自动生成、结构校验和必须人工复核的边界。
- 正式公文核验表：模板完成后的全要素检查。
- 生成器：根据输入生成 DOCX 版式。
- 校验器：检查生成器可承诺的 DOCX 版式要素。
- 回归测试：运行居中与单双页页码生成、DOCX 包结构与 PDF 转换验证。
- 全条款视觉核验：实际生成 18 份 DOCX、37 个 PDF 页面和 PNG，并逐条记录截图证据与人工边界。
- 全条款截图证据矩阵：把 GB/T 9704-2012 每个条款绑定到实际截图，明确哪些项目截图不能证明。
- 版头坐标量测：用 PDF 字形坐标独立核对份号首行、机关标志 35 mm 定位及可选字段不移位。
- 跨平台安装测试：隔离环境中验证九个本地目录和 TraeWork 导入包。

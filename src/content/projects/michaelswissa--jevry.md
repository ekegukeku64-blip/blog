---
title: "michaelswissa/jevry"
owner: "michaelswissa"
name: "jevry"
fullName: "michaelswissa/jevry"
description: "Your browser. Ready to act. An MIT-licensed desktop browser agent for website tasks, cited research, and supported games."
sourceUrl: "https://github.com/michaelswissa/jevry"
stars: 100
forks: 0
language: "TypeScript"
topics: ["ai-agents", "browser-automation", "desktop-app", "electron", "research", "typescript"]
license: "MIT"
defaultBranch: "main"
snapshotDate: "2026-09-25"
pushedAt: "2026-09-24T20:04:18Z"
---

> 本页保存的是公开项目资料快照，阅读过程不需要连接 GitHub。

A browser built around Jev.

Language models plan. Jev decides. Chromium acts.An open-source desktop browser that turns natural-language tasks intostructured decisions, guarded actions, and observable results.

*图片：Watch the Jevry introduction video*
...
*图片：Latest Jevry workspace with the full chrome J artwork, updated conversation sidebar, and browser navigation*

*Latest desktop design, captured from the local September 24 build.*


Watch Jevry win 2048 — recorded with the earlier interface

*图片：Historical real-time game excerpt: Jevry makes the final moves and wins 2048*
...
Watch the 30-second real-time clip · Recording and edit details
...
**Source preview · 0.4.0-beta.13 · macOS and Windows CI.** Both platforms have passed native fixture workflows and packaged-app checks in the public launch CI run. Real-provider experience is primarily tested on macOS. Signed installers, notarization, and automatic updates remain future work.
...
*图片：Latest Jevry setup screen with the chrome J artwork and Codex, Claude Code, and API-key connection choices*
...
Keys go into the app, not source files. Setup validates the connections. Detailed provider behavior · Security and data flow
...
Build a local app, update, or troubleshoot setup
...
Still stuck? Open an issue with OS, version, steps, and a redacted error.
...
HTML/ARIA controls, native selects, contenteditable fields, open shadow roots, nested scrolling, and bounded same-origin iframe interactions are supported. Detailed interaction coverage
...
**Go deeper:** Jev decision architecture · Engine internals · Research pipeline · Conversation lifecycle
...
All-attempt score table and public JSON · All development attempts, failures, and methodology
...
One successful instrumented run; initial setup still took tens of seconds. The supplied launch video is another recording with different totals. Full game results, interrupted attempts, and build identities


Shared-task comparison: Jevry and an adapted Firecrawl source graph
...
This compares actual Firecrawl source orchestration with an injected local-browser toolkit and Claude bridge, **not its hosted product**. Browser versions/viewports differed, two native probes overlapped Firecrawl sessions, and three repetitions cannot establish statistical significance. Browser launch and initial fixture navigation were excluded. Ranges, raw-score caveats, and historical v0.3 results
...
The public launch CI run passed on **macOS and Windows**, including builds, native desktop/research/verification fixtures, Chromium action tests, and packaged-app checks. Local launch validation recorded 571 default test passes with 100 opt-in skips, plus 120 selected Chromium checks; these counts overlap.
...
These workflows use disposable profiles and deterministic providers. They verify mechanics; live model quality is measured separately. Evaluation protocol · Performance methodology

## Engineering worth inspecting

| Design decision | Why it matters | Start reading |
| --- | --- | --- |
| Complete action choices with fan-out fallback | Keeps operation/target relationships explicit while bounding choice cardinality. | Action compiler |
| Isolated, document-bound observations | Stops stale references from silently targeting a changed page. | Snapshot, native evaluator |
| Coverage-aware completion | Distinguishes one visible match from a complete answer across requested records. | Task completion, observation memory |
| Receipt-preserving cancellation | Keeps the record of dispatched input and avoids replay after uncertain mutations. | Engine, turn lifecycle |
| Local numeric perception | Keeps repetitive board reading out of the general vision loop. | Game perception, OCR, forecasting |
| Separate actor and evaluator | Lets official outcome checks disagree with the agent's own completion claim. | Benchmark input boundary, runner |
...
Open an issue before a large change. Include what changed, why, and the checks you ran in your PR. Browser tests must use disposable profiles; game changes should preserve the documented beta.4 baseline. Contribution guide · Report a bug · Start a discussion
...
Connections and conversation archives use OS-backed local encryption, while connected providers receive relevant prompts/page context and, for supported visual tasks, images. Jevry uses its own persistent browser profile. **Local storage does not imply offline inference.** Security policy
...
Created by Michael Swissa. Jevry is an independent project built around **TypeSafe's Jev model**; Michael did not train or create Jev.

Jevry adapts Jev Ultrafast's MIT-licensed snapshot and action-loop design into a persistent Electron browser, extending it with conversations, research, guarded visual/game control, completion evidence, and evaluation infrastructure. Firecrawl's public source and Ego Lite's interaction patterns also informed the architecture. Upstream revisions and integration boundaries

Original Jevry code is MIT licensed. Third-party code, fonts, and packages retain their own licenses in THIRD_PARTY_NOTICES.md. This project is not an official product or endorsement of TypeSafe, Browser Use, Anthropic, or OpenAI.

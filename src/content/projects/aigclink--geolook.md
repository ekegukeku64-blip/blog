---
title: "aigclink/geolook"
owner: "aigclink"
name: "geolook"
fullName: "aigclink/geolook"
description: "Open-source end-to-end GEO implementation: status analysis, diagnosis, strategy, tickets, execution, verification"
sourceUrl: "https://github.com/aigclink/geolook"
stars: 151
forks: 34
language: "Python"
topics: ["aeo", "ai-search", "ai-visibility", "answer-engine-optimization", "generative-engine-optimization", "geo", "llm", "self-hosted"]
license: "MIT"
homepage: "https://geolook.cc"
defaultBranch: "main"
snapshotDate: "2026-08-01"
pushedAt: "2026-07-31T05:10:47Z"
---
...
*图片：License* *图片：Python* *图片：Platform* *图片：Deps*

*图片：Product demo*

🌐 [Website geolook.cc](https://geolook.cc) · 🔍 [Live demo (read-only)](https://geolook.cc/demo/) · 📹 HD demo video (mp4) · 🖼 All screenshots

Mirror while DNS propagates: [geolook.cc](https://geolook.cc) · [demo](https://geolook.cc/demo/)
...
| **Scoring basis** | Heuristics | Anchored in public empirical data: 602 prompts / 21,143 citations / 187,818 deduplicated CN citations (references/) |
...
`--market` is `cn` / `global` / `both`. Nine steps run automatically: crawl → audit → derive facts/competitors/questions → sample every engine → tickets → assets → report → auto-verify → delivery package, landing in `work//delivery//`.
...
5. **Execute** — Take tickets P0-first in **Action Plan** (click a title for why/how/acceptance); write in the **Workbench** (pre-check ≥ B, publish as final, then work through the distribution checklist); deploy **Assets** (llms.txt to site root, JSON-LD into ``, snippets into templates — see DEPLOY.md).
...
```bash
python3 scripts/geo.py sample-sheet  --slug    # export sheet with per-question guidance
python3 scripts/geo.py sample-import --slug  --file 
```
...
All six audit dimensions are anchored in public empirical data; `scripts/audit.py` implements references/method.md:
...
This repo doubles as a Claude Code skill (SKILL.md): drop it into your skills directory and tell Claude "do GEO for example.com". Claude is optional — every script is a plain CLI.
...
tests/            Unit tests
work//      Per-project data (gitignored, never leaves your machine)
...
Questions, ideas or collaboration — email bingqiang2008@gmail.com, or open an issue.

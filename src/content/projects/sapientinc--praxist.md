...
Before starting research, read the Quickstart
and Your First Task. They describe the
...
open-source model APIs with a high
...
See Agent Skills for invocation syntax and the
generated Skills Reference for the complete
contracts.
...
| Quality-Diversity (QD) and optional Deep Innovation Gate (DIG) | Maintain diversity without forcing one exploration policy |
...
See the platform support matrix for the
...
Read the contribution guide together with the
Code of Conduct before participating. Source
maintainers should also follow the architecture and maintenance contract in
AGENTS.md. See the Privacy Notice for
...
Deep Innovation Gate (DIG) and
Quality-Diversity (QD) allocation help

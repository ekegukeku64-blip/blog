...
owner: "ghuntley"
name: "underclass"
fullName: "ghuntley/underclass"
description: "underclass: an OpenAI-compatible pooling proxy that pins sessions to one account (prompt cache stays warm), cools quota-exhausted subscriptions until their window resets, and fails fast with the earliest Retry-After when the whole pool is dry."
sourceUrl: "https://github.com/ghuntley/underclass"
stars: 121
forks: 5
language: "Rust"
...
Design decisions and their trade-offs live in `docs/adr/` — start with ADR 0010 for the routing model.
...
When preflight fronts underclass, it generates a UUIDv4 and forwards it as `x-request-id`. Underclass preserves that ID in its response, routing/retry logs, and request history. Search the same `request_id` in both services to trace inspection through pool routing.

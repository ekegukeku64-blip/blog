---
title: "Aqua-123/pstack-for-codex"
owner: "Aqua-123"
name: "pstack-for-codex"
fullName: "Aqua-123/pstack-for-codex"
description: "Codex-native engineering workflows derived from pstack, with 45 explicit skills and 23 Poteto Mode playbooks."
sourceUrl: "https://github.com/Aqua-123/pstack-for-codex"
stars: 47
forks: 1
language: "TypeScript"
topics: ["codex", "codex-plugin", "developer-tools", "engineering-workflows", "pstack"]
license: "MIT"
defaultBranch: "main"
snapshotDate: "2026-08-27"
pushedAt: "2026-08-26T10:28:35Z"
---
...
`pstack-for-codex` is a Codex-native derivative of pstack. It packages deliberate engineering workflows as 45 explicit-only skills and 23 Poteto Mode playbooks.
...
Start a new task after installation so Codex reloads the plugin catalog. See Set up pstack for the complete walkthrough.
...
Read Agent setup and model evidence before changing profiles.
...
| Skill | Use it for |
|---|---|
| `$how` | Trace how a subsystem works. |
| `$why` | Reconstruct why code reached its current shape from available evidence. |
| `$recall` | Rebuild recent project context through supported task history and live state. |
| `$architect` | Settle types, callers, and module boundaries before implementation. |
| `$arena` | Compare isolated attempts at the same brief. |
| `$swarm` | Cover independent slices or races and aggregate the result. |
| `$interrogate` | Run a skeptical, multi-lens review of a diff. |
| `$tdd` | Reproduce a bug with a failing test before fixing it. |
| `$no-comments` | Review comments and remove ones that do not earn their place. |
| `$unslop` | Remove vague or machine-shaped prose. |
| `$show-me-your-work` | Keep a reviewable `decisions.tsv` trail. |
| `$setup-benny` | Inspect or configure the dormant Benny polling pack. |

Browse the complete skill directory or read the pstack guide.
...
Benny is an optional polling pack for issue triage and reproduction. Installation does not create or activate an automation. `$setup-benny` copies the pack into a target project only after explicit authority.
...
NOTICE, `upstream.lock.json`, and the compatibility map record the source commit, license, file hashes, and migration status. UPSTREAM.md explains how maintainers refresh that evidence through a temporary local source checkout.

See Codex adaptation notes for the behavioral changes and current limits.

---
title: "Electricitysheep/dsh-handbook"
owner: "Electricitysheep"
name: "dsh-handbook"
fullName: "Electricitysheep/dsh-handbook"
description: "DeepSeek Harness (dsh) 从 0 到 1 深度手册：安装/插件开发/性能调优/实测案例/同模型多 Agent 实测对比（中文 + 英文 PDF）"
sourceUrl: "https://github.com/Electricitysheep/dsh-handbook"
stars: 338
forks: 9
language: "HTML"
topics: ["agent", "agent-framework", "ai-agents", "beginners", "deepseek", "deepseek-ai", "dsh-plugin", "getting-started"]
license: "未标注"
defaultBranch: "main"
snapshotDate: "2026-08-16"
pushedAt: "2026-08-16T10:23:47Z"
---
...
> **DeepSeek Harness 中文手册 × 生态观察中心**——从 0 到 1 玩转 dsh，跟着 780 帖讨论区看懂生态 · 中文 · English

**📖 [在线阅读](https://electricitysheep.github.io/dsh-handbook/) · 📄 下载 PDF · ⭐ 点 Star 支持**
...
> [!WARNING]
> dsh 当前为 `0.1.0-rc.6`（预发布阶段），生产环境请谨慎评估，详见 ℹ️ 版本说明。
...
> 想系统学？看 🗺 学习路径（3 天计划）；想先跑？第 2 章：五分钟快速上手；想速查？📇 一页速查卡


  
  
  30 秒看懂 dsh Web UI：新建会话 → 输入任务 → 模型选择 → 发送 → AI 回复
...
- 💬 **官方库 195 帖回应**——我们持续在讨论区和大家一起踩坑、排障、交流
- 🧠 **FAQ 里的 39 条问题大多来自真实提问**——社区问什么，我们沉淀什么（#380/#817/#1052…）
- 📦 已向 awesome-dsh-plugin 提交收录 PR（#33，待合并）；阮一峰周刊自荐已提交
- 🤝 与 30+ 社区项目互链（dsh-usage / dsh-sgme / AgentSoul / dsh-vault / egress-guard / agentmemory…）

> 内容随讨论区持续更新（沉淀流水线，19 项可追踪）。如果你觉得有用，Star 是对我们最大的支持。
...
> 完整报告（含对开发者/选型者/观望者的建议）：第 15 章

## 📚 目录（从 0 到 1）


| 🗺️ **学习路径（3 天计划）** | 从 0 到 1：每天目标 + 验收标准 + 学习原则 |
|---|---|
...
| 📖 **第 1 章 · 认识 Harness** | ⚡ **第 2 章 · 五分钟上手** |
|---|---|
| 与主流 Agent 全面对比 · FAQ · EN | 安装 · web/headless 双模式 · 推理档位 · EN |


### 🔵 阶段 2 · 开发：骨架与插件


| 🧩 **第 3 章 · profile 与插件系统** | 🛠️ **第 4 章 · 插件开发实战** |
|---|---|
...
| 🌐 **第 7 章 · 生态与资源** | 🧰 **第 8 章 · 工具与上下文系统** | 🔗 **第 9 章 · MCP 子代理与工作流** |
|---|---|---|
...
| 📚 **附录 A·术语表** · 📦 **附录 B·官方包速查** · 📊 **附录 C·Benchmark** |
|---|
...
运行 `dsh --profile headless "你好，请用一句话介绍你自己"`（命令见 🚀 快速体验）：
...
*图片：dsh Git 面板（better-sidebar 插件）*
...
```
> 可克隆模板（含纯函数+waterfall+测试）：examples/plugin-template/
...
```
> 全字段（profile/cordis.patch.yml/常用场景）：docs/config-reference.md
...
> 5 任务 × 3 轮采样中位数（T1 创建文件 → T5 多文件重构），45/45 全对。完整方法/解读见 📊 Benchmark 附录。
...
- **中文完整版**：DeepSeek-Harness-白皮书.pdf（15 章 + 附录 ABC，~130k+ 字符，5.5MB）
- **英文完整版**：DeepSeek-Harness-Handbook.pdf（15 章 + 附录，83 页，约 160k 字符，1.7MB）
...
### 🧩 社区插件推荐（来自官方讨论区 / awesome-dsh-plugin 精选）

| 插件 | 用途 |
|---|---|
| dsh-specflow | 规格驱动开发：技能+命令+目标实现+进度上下文 |
| dsh-gitflow | 审批门控的 Git 工作流（status/diff/commit/branch） |
| dsh-guardian | 危险操作策略检查 + 输出脱敏 + 安全审查 |
| dsh-code-intel | Tree-sitter 符号索引 + 混合搜索 |
| dsh-tianshu-tui | 终端 UI（TUI） |
| dsh-computer-use | 无障碍优先的 macOS 电脑控制 |
| dsh-data-agent | 连数据库写 SQL 的数据 Agent |
| dsh-balance-meter | 余额 + 会话成本实时显示 |
| dsh-usage | Token 用量 + 费用估算 + 52 周热力图（#1169） |
| dsh-sgme | 记忆引擎：按场景注入 + 自动剪枝（省 65-96% 会话，#1052） |
| AgentSoul | 本地人格 + 长期记忆 + 记忆蒸馏（#1478） |
| dsh-vault | 加密凭据保险库：TOTP/API Key/SSH 加密存储（#1457） |

> 完整列表见 awesome-dsh-plugin（122+ 插件）。想被收录？社区案例征集

### 📣 官方讨论区活跃响应

已在 deepseek-ai/deepseek-harness Discussions 持续活跃响应（100+ 帖）：插件踩坑 #380 / 安全审计 #817 / 缓存成本 #1052/#1234 / 生态洞察帖 #839 等（沉淀流水线 持续收录反馈）

## 🙏 贡献与反馈

- ⭐ 觉得有帮助？点个 Star 支持持续更新
- 📝 **跑过真实案例？** 投稿收录进白皮书（署名 + 季度精选 PDF）：社区案例征集 ← 直接回帖，模板已备好
- 章节/命令失效？rc 版本迭代所致，欢迎 issue 指正
- 想参与？见 🤝 贡献指南（CONTRIBUTING） · 想看计划？🗺️ 路线图（ROADMAP） · 生态参与见 第 7 章
...
- 讨论区反馈持续沉淀：FAQ 39 条 / KVCache 规则 / 内置 Agent 预设 / run_code 异步坑（沉淀流水线）
